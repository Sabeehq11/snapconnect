...
RAGService.generateTextSuggestions = async function(userId, context) {
  try {
    console.log('🤖 RAG: Generating text suggestions for user', userId);
...
Return only the 5 messages, one per line, without numbering.`;
    ...
    console.log('🔍 RAG: Raw text suggestions response:', response);
    ...
    console.log('✅ RAG: Parsed text suggestions:', suggestions);
    return suggestions;
  } catch (error) {
    console.error('❌ RAG: Error generating text suggestions:', error);
    throw error;
  }
};
